...
/////LAB1////////
//класс 3 круга
class CTripleCircle : public CBasePoint
{
	DECLARE_SERIAL(CTripleCircle)

protected:
	WORD m_wSize; // Диаметр одного круга

public:
	// Конструкторы
	CTripleCircle(int x, int y, WORD s);
	CTripleCircle();
...
class CPolygon: public CBasePoint
{
 ...
 protected:  
	 // Метод сериализации
	void Serialize(CArchive& ar);
	CBasePoint* m_SelectedShape; // Указатель на выбранную фигуру
 public:
...
//Методы
	...
	bool IsInsideRect(const CRect& selectionRect) const;
};
...
////////////////////////////////////////////////
//класс звездочка
class CStar : public CPolygon// CBasePoint//CPolygon
{
	DECLARE_SERIAL(CStar)
protected:
	void InitializePoints(int x, int y, int size);
	// Метод сериализации
	void Serialize(CArchive& ar);
public:
	// Конструкторы
	CStar(int x, int y, WORD s);
	CStar();
	~CStar() {};

	// Методы
	// Отображает фигуру звезды на экране
	void Show(CDC* pDC) override;
	// Сообщает область захвата фигуры
	void GetRegion(CRgn& Rgn) override;
};
