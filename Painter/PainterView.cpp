...
BEGIN_MESSAGE_MAP(CPainterView, CScrollView)
	...
	/////////LAB1/////////////////
	ON_COMMAND(ID_EDIT_ADDSHAPE_TripleCircle, OnEditAddshapeTripleCircle)
	/////////LAB2/////////////////
	//Добавила команду для рисования звезды
...
CPainterView::CPainterView()
{
	...
	m_hcurCursor = AfxGetApp()->LoadCursor(IDC_CURSOR_MODE);
}
...
void CPainterView::AddShape(int shape, CPoint first_point, CPoint second_point) 
{
	...
	case OP_TRIPLECIRCLE: //LAB1
		// Создаем объект - квадрат
		pShape = new CTripleCircle(first_point.x, first_point.y, size * 2);
		// Красная линия шириной 1 мм
		pShape->SetPen(RGB(200, 0, 0), 100, PS_GEOMETRIC);
		// Закрашвиаем
		//pShape->SetBrush(RGB(200, 0, 0), 0);
		break;
	case OP_STAR: //LAB2
		// Создаем объект - звезду
		pShape = new CStar(first_point.x, first_point.y, size * 2);
		// Настраиваем перо для контура звезды
		pShape->SetPen(RGB(200, 200, 0), 100, PS_GEOMETRIC);
		// Настраиваем кисть для заполнения звезды (например, светло-желтый цвет)
		pShape->SetBrush(RGB(255, 255, 0), 0);
		break;
...
/////////LAB1///////
void CPainterView::OnEditAddshapeTripleCircle()
{
	m_CurOper = OP_TRIPLECIRCLE;
}
...
{
	m_CurOper=OP_SELECT;
	::SetClassLong(GetSafeHwnd(), GCL_HCURSOR, (LONG)m_hcurCursor);
}


///////////////LAB2////////////////////
//Изменила код, чтобы вокруг фигуры рисовался пунктирный прямоугольник
void CPainterView::MarkSelectedShape(CDC *pDC)
{
	CPainterDoc* pDoc = GetDocument();
	if (pDoc->m_pSelShape == NULL) return;

	// Сохраняем текущее перо и кисть
	CPen* pOldPen = pDC->GetCurrentPen();
	CBrush* pOldBrush = pDC->GetCurrentBrush();

	// Создаем пунктирное перо и прозрачную кисть
	CPen dashedPen(PS_DASH, 1, RGB(0, 0, 0));
	CBrush hollowBrush;
	hollowBrush.CreateStockObject(HOLLOW_BRUSH);

	// Устанавливаем пунктирное перо и прозрачную кисть
	pDC->SelectObject(&dashedPen);
	pDC->SelectObject(&hollowBrush);

	// Определяем область фигуры и получаем ее прямоугольные границы
	CRgn Rgn;
	...
	if (res != ERROR && res != NULLREGION) {
		// Рисуем прямоугольник рамки вокруг фигуры
		pDC->Rectangle(&Rect);
	}

	// Восстанавливаем предыдущее перо и кисть
