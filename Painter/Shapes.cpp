...
///////////////////////////////////////
////////LAB1////////////////////
// Реализация методов CTripleCircle

IMPLEMENT_SERIAL(CTripleCircle, CObject, 1)

CTripleCircle::CTripleCircle(int x, int y, WORD s) : CBasePoint(x, y, s) {
	m_wSize = s;
}

CTripleCircle::CTripleCircle() : CBasePoint() {
	m_wSize = 40; // Устанавливаем размер по умолчанию
}

void CTripleCircle::Serialize(CArchive& ar) {
	CBasePoint::Serialize(ar);
}

void CTripleCircle::Show(CDC* pDC) {
	int radius = m_wSize / 2;          // Радиус круга
	int spacing = radius * 2 + 10;    // Расстояние между центрами кругов (диаметр + зазор)

	// Устанавливаем перо для обводки (например, черный контур)
	CPen pen(PS_SOLID, 1, RGB(0, 0, 0));
	CPen* pOldPen = pDC->SelectObject(&pen);

	// Рисуем три круга с разными цветами
	for (int i = -1; i <= 1; ++i) {
		int centerY = y + i * spacing - 10; // Центр каждого круга

		// Выбираем цвет для текущего круга
		COLORREF color;
		if (i == -1) color = RGB(255, 0, 0);    // Красный для верхнего круга
		else if (i == 0) color = RGB(255, 255, 0); // Желтый для среднего круга
		else color = RGB(0, 255, 0);           // Зеленый для нижнего круга
		...
		// Рисуем круг
		pDC->Ellipse(x - radius, centerY - radius, x + radius, centerY + radius);

		// Восстанавливаем предыдущую кисть
		pDC->SelectObject(pOldBrush);
	}

	// Восстанавливаем предыдущий перо
	pDC->SelectObject(pOldPen);
}


void CTripleCircle::GetRegion(CRgn& Rgn) {
	// Создаем сложный регион из трех кругов
	int radius = m_wSize / 2;
	int spacing = radius + 10;

	// Создаем регионы для каждого круга
	CRgn circle1, circle2, circle3;
	circle1.CreateEllipticRgn(x - radius, y - spacing - radius, x + radius, y - spacing + radius);
	circle2.CreateEllipticRgn(x - radius, y - radius, x + radius, y + radius);
	circle3.CreateEllipticRgn(x - radius, y + spacing - radius, x + radius, y + spacing + radius);

	// Объединяем регионы
	Rgn.CreateRectRgn(0, 0, 0, 0); // Пустой регион
	Rgn.CombineRgn(&circle1, &circle2, RGN_OR);
	Rgn.CombineRgn(&Rgn, &circle3, RGN_OR);
}
...
///////////////////////////////////////
////////LAB2////////////////////
// Реализация методов класса Star

IMPLEMENT_SERIAL(CStar, CPolygon, 1)

CStar::CStar(int x, int y, WORD s) : CPolygon()
{
	m_wSize = s;
	InitializePoints(x, y, s);
}

CStar::CStar() : CPolygon()
{
	m_wSize = 40;
	InitializePoints(0, 0, m_wSize);
}

// Инициализируем точки звезды
void CStar::InitializePoints(int x, int y, int size)
{
	const int numPoints = 10; // Количество вершин звезды
	double angleStep = M_PI / 5; // Шаг угла для 10-лучевой звезды
	int radius = size / 2;
	...
	for (int i = 0; i < numPoints; ++i)
	{
		...
		int currentRadius = (i % 2 == 0) ? radius : radius / 2; // Чередование длин лучей
		CPoint point;
		point.x = x + static_cast<int>(currentRadius * cos(angle));
		point.y = y - static_cast<int>(currentRadius * sin(angle));
...
{
	CPolygon::Serialize(ar); // Сериализация с помощью базового класса
}

void CStar::Show(CDC* pDC)
{
	PrepareDC(pDC);
	if (m_PointsArray.GetSize() > 0)
	{
...
void CPolygon::Transform(const CPoint &point0, double ang, int dx, int dy)
{
	for(int i=0; i<m_PointsArray.GetSize(); i++)
		m_PointsArray[i]=::Transform(m_PointsArray[i], m_PointsArray[0], ang, dx, dy);
};


bool CPolygon::IsInsideRect(const CRect& selectionRect) const
{
	for (int i = 0; i < m_PointsArray.GetSize(); ++i) {
		if (selectionRect.PtInRect(m_PointsArray[i]) == FALSE) {
			return false;
		}
	}
	return true;
